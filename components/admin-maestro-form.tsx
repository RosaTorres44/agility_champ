...
export function DynamicForm({ entityType = "Entidad", reloadData, selectedEntity, onCancel }: DynamicFormProps) {
  const form = useForm<z.infer<typeof entidadFormSchema>>({
    ...
    defaultValues: { nombre: "", flg_activo: true },
    mode: "onChange",
  });
  ...
  useEffect(() => {
    if (selectedEntity) {
      ...
      form.reset({ nombre: "", flg_activo: true });
    }
  }, [selectedEntity, form]);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField control={form.control} name="flg_activo" render={({ field }) => (
          <FormItem className="flex flex-row items-center justify-between border p-4">
            <FormLabel className="text-base">Activo</FormLabel>
            <FormControl>
              <Switch checked={field.value} onCheckedChange={(checked) => form.setValue("flg_activo", checked)} />
            </FormControl>
          </FormItem>
        )} />
